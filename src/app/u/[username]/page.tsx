...
import { FeaturedFilmsSection } from "@/components/profile/FeaturedFilmsSection";
...
export default async function PublicProfilePage({
  ...
  const featuredFilms = canViewActivity
  ...
  const { users: mutualFollowers, total: mutualFollowersCount } =
    viewerId && !isOwner
  ...
  const avatarUrl = getUserAvatarUrl(user.avatarPath, user.image);
  const socialConnection = getSocialConnection(user.socialLink);
  ...
  return (
    <main className="min-h-screen bg-[#100E0C] text-[#F4F1EB]">
      ...
      <div className="mx-auto max-w-7xl px-5 py-10 md:px-8 md:py-14">
        {/* Profile header — avatar/info left, follow section right */}
        ...
        {/* Full-width sections below the profile header */}
        {canViewActivity ? (
...
async function loadPublicActivity(userId: string): Promise<
  [ProfileDiaryEntry[], number, number, number | null]
> {
  const [
    entries,
    publicEntryCount,
...
async function loadMutualFollowers(
...
): Promise<{ users: MutualFollower[]; total: number }> {
  const mutualWhere = {
    followingId: profileUserId,
    status: FollowStatus.ACCEPTED,
    followerId: { not: viewerId },
    follower: {
      onboardingCompleted: true,
      deletedAt: null,
      suspendedAt: null,
  ...
  const [rows, total] = await Promise.all([
    ...
    prisma.follow.count({ where: mutualWhere }),
  ]);
    ...
    >
      {!posterUrl ? (
        <div className="flex h-full items-center justify-center p-2 text-center">
          <span className="text-xs font-medium text-[#8A8580]">{title}</span>
        </div>
      ) : null}
    </div>
  );
  ...
  return (
    <div className="mt-5 flex items-center gap-3">
      ...
      <p className="min-w-0 text-xs text-[#8A8580]">
        ...
        >
          {firstUser.name?.trim() || `@${firstUser.username}`}
        </Link>
        {remainingCount > 0
...
function getSocialConnection(
  ...
  try {
    ...
    if (hostname === "github.com") {
      return { url: url.toString(), platform: "github", label: pathParts[0] || "GitHub" };
    }

    if (hostname === "instagram.com") {
      return { url: url.toString(), platform: "instagram", label: pathParts[0] || "Instagram" };
    }

    if (hostname === "x.com" || hostname === "twitter.com") {
      return { url: url.toString(), platform: "twitter", label: pathParts[0] || "X" };
    }

    if (hostname === "tiktok.com") {
      return { url: url.toString(), platform: "tiktok", label: pathParts[0]?.replace(/^@/, "") || "TikTok" };
    }

    if (hostname === "open.spotify.com") {
      return { url: url.toString(), platform: "spotify", label: "Spotify" };
    }

    return { url: url.toString(), platform: "website", label: hostname };
  } catch {
    return null;
  }
}
