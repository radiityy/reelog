...
import {
  deleteDiaryPublishedNotifications,
  notifyFollowersAboutDiary,
...
export async function PATCH(
  ...
  const validationResult =
    updateDiaryEntrySchema.safeParse(
      requestBody,
    );
  ...
  try {
    ...
    if (becamePublic) {
      try {
        ...
        console.error(
          "Failed to notify followers about published diary:",
          notificationError,
        );
      }
    }
    ...
    if (becamePrivate) {
      try {
        await deleteDiaryPublishedNotifications({
          actorId: user.id,
          diaryEntryId: diaryEntry.id,
        });
      } catch (notificationError) {
        console.error(
          "Failed to remove diary notifications:",
          notificationError,
        );
      }
    }
    ...
    return NextResponse.json(
      {
        message:
          "Unable to update this diary entry.",
      },
      {
        status: 500,
      },
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      {
        message:
          "Unable to delete this diary entry.",
      },
      {
        status: 500,
      },
